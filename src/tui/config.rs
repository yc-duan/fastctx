...
use crate::control::settings::{FastCtxSettings, Tier, ToolBudgetLevel, ToolBudgets};
...
const CONFIG_GROUPS: [ConfigGroupSpec; 2] = [
    ConfigGroupSpec {
...
pub(crate) fn group_title(group: ConfigGroupId, messages: &Messages) -> &'static str {
    match group {
...
mod tests {
    ...
    fn cursor_preserves_group_parent_child_order_and_wraps() {
        ...
        assert_eq!(cursor.previous().entry().item, ConfigItemId::JobListLimit);
    }
    ...
    fn tab_navigation_always_lands_on_a_group_parent() {
        ...
        assert_eq!(extensions.next_group(), output);
        assert_eq!(output.previous_group(), extensions);
    ...
    fn viewport_keeps_focus_visible_and_reports_both_hidden_edges() {
        ...
        while cursor.entry().item != ConfigItemId::JobListLimit {
            cursor = cursor.next();
        }
