...
每个 `fastctx serve` 进程都是一个轻量 stdio 代理。同一用户、同一 FastCtx 构建的代理共享一个私有本地控制中心，包括它的搜索执行器和全局准入上限；而每条 MCP 连接各自保留自己的工作目录、原生环境、取消状态和后台输出游标。
...
- `inspect_local_file` 读取文本、图片、PDF 和原始字节；
- `grep` 搜索文件内容；
- `glob` 查找文件；
- `replace` 执行机械批量替换；
...
grep/glob 默认使用自动并行度：操作系统报告的可用并行度，最高 16。在 **配置 → 搜索** 中，可用 ←/→ 选择预设，也可按 Enter 输入 `auto` 或界面显示的 `1..=最大值` 范围内任意整数。该设置在共享控制中心启动时读取；控制中心重启后生效，而控制中心要等用过它的 Codex 进程全部退出才会重启。不需要重新接入。
...
上表描述的是**未登记进 World 的机器**，也就是你自己动手登记之前的每一台机器。登记是一个明确的动作——用你自己签发的邀请跑一次 `fastctx node enroll`——它只增加一条对外连接，由 `fastctx node` 守护进程发起，连向一台你自己运行、自己拥有的中枢。`fastctx serve` 本身仍然不发起任何网络请求。中枢按明文信头路由，读不到成员之间的内容：消息正文由一把中枢没有的 World 密钥加密。这条路径上没有 FastCtx 的服务器，也没有任何厂商的服务器；不会上传你没要求的东西——一次跨机调用带走的是你传的参数，带回的是那次调用产生的输出。
