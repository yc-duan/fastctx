...
grep/glob 默认使用自动并行度：操作系统报告的可用并行度，最高 16。在 **配置 → 搜索** 中，可用 ←/→ 选择预设，也可按 Enter 输入 `auto` 或界面显示的 `1..=最大值` 范围内任意整数。每个新启动的 MCP server 会直接读取这个设置；重启 server 后生效，不需要 Apply。
...
**配置 → 重置 → 重置全部设置** 的确认页默认选中 **No**。确认后，语言、输出预算、Bash/job 限制、搜索 CPU 上限和更新设置等全部用户偏好都会恢复默认；Apply 所有权回执、已安装二进制、宿主配置与运行中的 job 保留不动。恢复默认 1024 MiB 历史留存配额时，超出配额的最旧 finished 记录可能立即被正常回收。
...
`job_output` 查询一个后台任务（也能查询此前会话启动的任务），返回 `running`、`exited` 或 `interrupted` 状态，以及调用方尚未看到的最新输出。`wait_ms`（0–60000，默认 30000）是这次查询最多可以花多久：任务结束即返回，否则等满这个窗口——因为每来一行就返回一次，等于用一整个回合去汇报"构建还在编译"。需要立刻拍快照就传 `wait_ms=0`。当前格式的输出过长时按窗口交付——装得下的最新若干行，加上首次调用时日志开头的一段——并由一条注给出被跳过的确切行号区间和可供读取的日志路径。该日志的行号与 `after_seq` 使用的 `seq` 是同一套编号，因此在两个工具之间移动无需换算。上一版分段格式写出的记录仍可读取，也兼容旧监督进程继续追加；但它不会假装拥有直接日志坐标，旧环形窗口已经淘汰的字节也无法恢复。

持续调用，直到结果末行显示 `Complete`。当前格式后台任务的日志会保留每一行，所以响应里没展示的内容，一次 `read` 或 `grep` 就能取回；只有上一版格式创建的存量记录受前述兼容边界限制。
...
- `Standard`：默认档，宿主上限 16000，FastCtx 预算 13600；
- `High`：宿主上限 25000，FastCtx 预算 21250。
