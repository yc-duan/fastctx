...
`job_output` 增量读取后台任务的新输出，也能读取此前会话启动的任务，并返回 `running`、`exited` 或 `interrupted` 状态。`wait_ms` 最长可长轮询 240000 ms，默认 30000 ms。默认 `wait_for="output"`，新输出或终态任一到达就返回；构建、测试等只关心结束的任务应使用 `wait_for="exit"`，中间行只会累积，不会提前结束等待，到达终态或等满 `wait_ms` 时再一并返回。`after_seq` 可以重新锚定读取位置，在调用重试时保持分页稳定。
